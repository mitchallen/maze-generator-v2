...
`grid` is a local workspace package providing 2D grid implementations (Square, Circle, Hexagon, Triangle) with zero-based indexing. It targets both Node.js and browsers via browserify. Hexagon and Triangle are currently mapped to the Square implementation.
